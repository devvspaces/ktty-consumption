...
const MINTING_ABI = [
    "function batchAddBooks(uint256[] calldata bookIds, uint256[] calldata nftIds, uint256[3][] calldata toolIds, uint256[] calldata goldenTicketIds, string[] calldata nftTypes) external",
...
function validateBook(book, index) {
    const requiredFields = ['bookId', 'nftId', 'nftType', 'toolIds', 'goldenTicketId', 'hasGoldenTicket'];
    ...
    if (typeof book.nftType !== 'string' || book.nftType.trim() === '') {
        throw new Error(`Book ${book.bookId} has invalid nftType`);
    }
}
 ...
 */
async function loadBooksWithRetry(contract, batchData, batchNumber, maxRetries = 3) {
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
        try {
            console.log(`📤 Sending batch ${batchNumber} (attempt ${attempt}/${maxRetries})...`);
            
            const tx = await contract.batchAddBooks(
                batchData.bookIds,
                batchData.nftIds,
                batchData.toolIds,
...
async function loadBooks() {
    ...
    for (let batchIndex = 0; batchIndex < batches.length; batchIndex++) {
        ...
        const result = await loadBooksWithRetry(contract, batchData, batchNumber);
